...
export async function createWorkshopAction(formData: FormData) {
  ...
  if (!existing) await repository.createWorkshop({ ownerUserId: user.id, ...input });
  redirect("/");
}
...
export async function createVehicleAction(formData: FormData) {
  ...
  if (!await repository.findCustomer(workshopId, input.customerId)) throw new Error("Registro não encontrado");
...
export async function recordServiceAction(formData: FormData) {
  ...
  const input = parse(serviceSchema.safeParse({
    ...raw,
    inventory: inventoryItemId && quantity > 0 ? [{ inventoryItemId, quantity }] : [],
  }));
...
export async function adjustInventoryAction(formData: FormData) {
  const { workshopId, repository } = await getOwnerContext();
  const input = parse(inventoryAdjustmentSchema.safeParse(formObject(formData)));
...
export async function recordContactAction(formData: FormData) {
  ...
  const draft = buildWhatsAppDraft({ phone: customer.phone ?? null, consent: customer.whatsappConsent ?? false, customer: customer.name, workshop: workshop.name, reason });
  if (!draft) throw new Error("Cliente sem telefone válido ou consentimento para WhatsApp.");
  await repository.recordContact({ workshopId, customerId, reason, message: draft.message });
  redirect(draft.url);
}
