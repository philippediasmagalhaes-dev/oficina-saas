...
export default async function ServicesPage() {
  ...
  return <AppShell workshopName={data.workshop.name} current="/servicos">
    ...
    <section className="surface" id="novo"><div className="section-title"><div><span>Novo lançamento</span><h2>Registrar serviço concluído</h2></div></div>{data.customers.length ? <ServiceForm customers={data.customers} inventory={data.inventory} /> : <EmptyState title="Cadastre um cliente primeiro" description="O serviço precisa estar vinculado a um cliente da sua oficina." action="Ir para clientes" href="/clientes" />}</section>
